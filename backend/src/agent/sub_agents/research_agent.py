    ...
    if uploaded_files:
        ...
        for f in uploaded_files:
            if isinstance(f, dict) and f.get("content"):
                ...
                file_parts.append(f"【文件: {f.get('title', '未知')}】\n{content_preview}")
        if file_parts:
            uploaded_files_text = "\n\n## 用户上传的内部资料\n" + "\n\n---\n\n".join(file_parts)
            logger.info(f"[KB] 注入 {len(uploaded_files)} 个上传文件内容到研究上下文")

    # 合并知识库上下文
    all_context = known_facts_text + uploaded_files_text
